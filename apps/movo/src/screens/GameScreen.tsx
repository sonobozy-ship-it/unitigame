...
function StarBar({ par, moves }: { par: number; moves: number }) {
  const stars = moves === 0 ? 3 : moves <= par ? 3 : moves <= par * 1.5 ? 2 : 1;
  return (
    <View style={{ flexDirection: 'row', gap: 4 }}>
      {[1, 2, 3].map(s => (
        <Text key={s} style={{ fontSize: 18, color: s <= stars ? COLORS.starActive : COLORS.starInactive }}>
          ★
        </Text>
      ))}
    </View>
...
export function GameScreen({ level, onWin, onBack }: GameScreenProps) {
  const { state, moveVehicle, undoMove, resetGame, getStars } = useGame(level);
  ...
  const handleMove = useCallback((vehicleId: string, direction: Direction, steps?: number) => {
    moveVehicle(vehicleId, direction, steps);
  }, [moveVehicle]);
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.controls}>
        ...
        <View style={styles.hintBox}>
          <Text style={styles.hintText}>Swipe penguins to slide them</Text>
        </View>
      </View>
    </SafeAreaView>
  );
